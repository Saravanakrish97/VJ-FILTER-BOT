  ...
  #  await message.react(emoji="♥️")
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(
            text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/MovieDiscussion24x7>🎥ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>\n\n<b>நீங்கள் இங்கிருந்து திரைப்படங்களைப் பெற முடியாது. எங்கள் <a href=https://t.me/MovieDiscussion24x7>🎥ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ல் கோருங்கள்.அல்லது கீழே உள்ள கோரிக்கை பொத்தானை கிளிக் செய்யவும்👇</b>\n\n\n\n",   
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/MovieDiscussion24x7")]])
        )
...
async def filter_episodes_cb_handler(client: Client, query: CallbackQuery):
  ...
  #  pre = 'filep' if settings['file_secure'] else 'file'
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="🙅‍♂️ 𝐍𝐎 𝐌𝐎𝐑𝐄 𝐏𝐀𝐆𝐄𝐒 𝐀𝐕𝐀𝐈𝐋𝐀𝐁𝐋𝐄",callback_data="pages")]
        )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
            ...
            try:
                ...
                await query.answer(f"Could not forward file. Please check the bot's permissions in the {FILE_FORWARD} channel.", show_alert=True)
            except Exception as e:
        ...
        # Assuming you have a function to get the file details based on the key
        files_ = await get_files_by_key(key)  # You need to implement this function
        if not files_:
            return await query.answer('Nᴏ sᴜᴄʜ ғɪʟᴇs ᴇxɪsᴛ.')

        try:
            for file in files_:
                file_id = file.file_id  # Assuming each file has a file_id attribute
                title = file.file_name
                f_caption = file.caption or title  # Use title if caption is not available

                # Check if is_shortlink is enabled
                if settings['is_shortlink']:
                    # You can modify the logic here if needed for shortlinks
            ...
            await query.answer('Files have been sent to the channel.', show_alert=True)
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/+MovieDiscussion24x7")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            ott = await query.message.reply_text(
                text=msg_text.format(get_name(log_msg), download, stream),
                quote=True,
                disable_web_page_preview=True,
    ...
    # don't change anything without contacting me @creatorrio
        ...
        free = get_size(free)
        await query.message.edit_text(
            text=script.STATUS_TXT.format(total, users, chats, premium, monsize, free),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        if users_id:
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/TamilSupport")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
                    InlineKeyboardButton("👮🏻‍♂️ Cᴏɴᴛᴀᴄᴛ", url="telegram.me/YaarulaNee")
                  ]]
...
async def advantage_spell_chok(client, msg):
    ...
    if AI_SPELL_CHECK == True:
        vj_ai_msg = await msg.reply_text("<b><i>Tamilini Ai Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
