...
async def link_handler(Client, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies6K")
        else:
            dump_file=await message.reply_video(url,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies6K")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies6K")
                   else:
                       dump_file=await message.reply_video(content_value, caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies6K")
               except:
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies6K")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies6K")
                  except:
